...
!DEM可変定数枠
real*8, parameter :: C = 0.158/0.123 !電子とイオンの温度比
...
!!プラズマ関連
integer, parameter :: kind = 5 !プラズマ種数
...
pl(1, 2) = 2.3445d11 !電子の電離圏数密度
pl(1, 3) = -1.d0 !電子の価数
pl(1, 4) = 1.23d-1 * e !1.5d3*kB !電子の電離圏温度(J)
pl(1, 5) = 1d0 * e !1.5d3*kB !電子の磁気赤道温度(J)

print *, pl(1, 4)

pl(2, 1) = 2.677950266103d-26 !酸素イオンの質量
pl(2, 2) = 2.269476d11 !酸素イオンの電離圏数密度
pl(2, 3) = 1.d0 !酸素イオンの価数
pl(2, 4) = pl(1, 4) * C !酸素イオンの電離圏温度(J)
pl(2, 5) = pl(1, 5) * C !酸素イオンの磁気赤道温度(J)

pl(3, 1) = 1.67262189821d-27 !水素イオンの質量
pl(3, 2) = 2.3445d8 !水素イオンの電離圏数密度
pl(3, 3) = 1.d0 !水素イオンの価数
pl(3, 4) = pl(1, 4) * C !水素イオンの電離圏温度(J)
pl(3, 5) = pl(1, 5) * C !水素イオンの磁気赤道温度(J)

pl(4, 1) = 6.64556638280897d-27 !ヘリウムイオンの質量
pl(4, 2) = 2.3445d8 !ヘリウムイオンの電離圏数密度
pl(4, 3) = 1.d0 !ヘリウムイオンの価数
pl(4, 4) = pl(1, 4) * C !ヘリウムイオンの電離圏温度(J)
pl(4, 5) = pl(1, 5) * C !ヘリウムイオンの磁気赤道温度(J)

pl(5, 1) = 2.325779481662744d-26 !窒素イオンの質量
pl(5, 2) = 7.0335d9 !窒素イオンの電離圏数密度
pl(5, 3) = 1.d0 !窒素イオンの価数
pl(5, 4) = pl(1, 4) * C !窒素イオンの電離圏温度(J)
pl(5, 5) = pl(1, 5) * C !窒素イオンの磁気赤道温度(J)
...
!電荷密度
do k = 1, kind
    charge = charge + nn(k, :)*dble(pl(k, 3))
end do !k
charge = charge * e


open(11, file = "DEM_E_L=4_vsPDS.csv")
do i = 1, N
 write(11, 21) lambda(i), ss(i), z(i), Ts(ss(i), sM, pl(1, 5), pl(1, 4), D)/e, nn(:, i), charge(i)
enddo !i

call Alfven(kind, N, pl, nn, lambda, Req, RE)


21 format(1PE25.15E3, 9(',', 1PE25.15E3))
...
subroutine ni(kk, kind, C, lambda, ss, sM, D, z, pl, HH, nn)
 ...
 nn = pl(kk, 2)*pl(1, 4)/Ts(ss, sM, pl(1, 5), pl(1, 4), D)
 nn = nn * exp(-C/(C+1.d0)*z/HH(1))
 nn = nn * exp(-z/HH(kk))
 nn = nn / EE**(C/(1.d0+C))
...
subroutine Alfven(kind, N, pl, nn, lambda, Req, RE)
 ...
 open(53, file = "DEM_E_L=4_vsPDS_VA.csv")
 do i = 1, N
