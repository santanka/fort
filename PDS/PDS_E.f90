...
integer :: MV = 110 !極小値(not parameter)
integer :: HATL = 110 !極大値(not parameter)
integer :: LATL = 110 !極小値(not parameter)、HATL <= LATL
integer, parameter :: kind = 10 !粒子種数
integer, parameter :: iodo = 5 !電離圏側の数密度調整
integer, parameter :: mado = 5 !磁気赤道側の数密度調整
...
real*8, dimension(3, N, kind, Z, 2) :: a2
...
!mag_FAの抽出
open(40, file="mag_FA_E_L=4_NS.csv", action="read", status="old")
do i = 1, N
...
!SVC_BCの抽出
open(20, file="PDS_BC_E_L=4_NS_3.csv", action="read", status="old")
read(20, *) !1行読み飛ばし
do i = 1, kind
...
open(33, file = "PDS_E_L=4_NS_min_3.csv")
read(33, *) DDD
!------------------------------
do i = 1, N
 read(33, *) pe0(i)
...
!つづきからはじめる▼
!open(15, file="PDS_E_penum.csv", action="read", status="old")
!do i = 1, N
! read(15, *) CC
! pe0(i) = CC
!enddo !i
...
!open(18, file="PDS_E_nsig.csv", action="read", status="old")
!do s = 1, kind
! read(18, *) sigma(s)
!enddo !s
!close(18)
...
72 format(1PE25.15E3, 13(',', 1PE25.15E3)) !kind+4
92 format(1PE25.15E3, 10(',', 1PE25.15E3)) !kind+1
 ...
 do s = 1, kind
  do j = 1, Z
   if(j == 1) then
     ...
     mu(s, j) = 1.d-30*(alpha*Tp(s)/BB(sigmab(s))/1.d-30)**(dble(j-2)/dble(Z-2))
   endif
  enddo !j
 enddo !s
...
subroutine EP(kind, N, ch, pe, GG, MassJ, mass, Req, LMD, omJ, dIo, MIo, UU, itn)
 ...
 if(itn == 1) then
   open(44, file="PDS_E_L=4_NS_phizeropote.csv")
   do i = 1, N
    write(44, 86) UU(i, :)
   enddo !s
   close(44)
 ...
 86 format(1PE25.15E3, 5(',', 1PE25.15E3)) !kind
end subroutine EP
...
subroutine aaa(kind, N, Z, alpha, Tp, Ta, UB, sigmab, a2)
    ...
    if(sigmab(s) == 1) then
      ...
      if(i /= 1) then
        ...
        if(UB(ud, i, s, j) > UB(1, t, s, j)) then
          a2(ud, i, s, j, 1) = sqrt(UB(ud, i, s, j) - UB(1, sigmab(s), s, j))
         else if(UB(ud, i, s, j) <= UB(1, t, s, j)) then
          a2(ud, i, s, j, 1) = sqrt(UB(1, t, s, j) - UB(1, sigmab(s), s, j))
        endif
      endif
    endif
  ...
  do i = 1, N
   do s = 1, kind
    do j = 1, Z
     ...
     !イオ起源
     if(sigmab(s) == 1) then
       if(i == N) a2(ud, i, s, j, 2) = 0.d0
       if(i /= N) then
         t = i+1
         do k = i+1, N
          if(UB(1, k, s, j) > UB(1, t, s, j)) t = k
         enddo !k
         a2(ud, i, s, j, 2)  = UB(1, t, s, j) - UB(1, sigmab(s), s, j)
         if(a2(ud, i, s, j, 2) < 0.d0) a2(ud, i, s, j, 2) = 0.d0
         if(a2(ud, i, s, j, 2) >= 0.d0) a2(ud, i, s, j, 2) = sqrt(a2(ud, i, s, j, 2))
       endif
     endif
     
     !木星起源
     if(sigmab(s) == N) then
       if(i == 1) a2(ud, i, s, j, 2) = 0.d0
       if(i /= 1) then
         t = 1
         do k = 1, i-1
          if(UB(1, k, s, j) > UB(1, t, s, j)) t = k
 ...
 open(70, file="PDS_E_a2.csv")
 do i = 1, Z
  write(70, 62) a2(1, 30, i, 30, :)
 enddo !i
 close(70)
...
subroutine TH(kind, N, Z, a2, Tp, Ta, theta)
 implicit none
 integer, intent(in) :: kind, N, Z
 real*8, dimension(3, N, kind, Z, 2), intent(in) :: a2
 real*8, dimension(kind), intent(in) :: Tp, Ta
  ...
  do i = 1, N
   do s = 1, kind
    do j = 1, Z
     ...
     !emit
     thetaL = 1-erf(a2(ud, i, s, j, 1)/sqrt(Tp(s)*Ta(s)))
     
     !bounce
     if(a2(ud, i, s, j, 2) > a2(ud, i, s, j, 1)) then
       thetaM = erf(a2(ud, i, s, j, 2)/sqrt(Tp(s)*Ta(s))) - erf(a2(ud, i, s, j, 1)/sqrt(Tp(s)*Ta(s)))
     endif
...
subroutine NN(kind, N, Z, iodo, mado, Tp, sigma, BB, sigmab, sigx, mu, theta, num)
 ...
 real*8, dimension(3, N, kind, Z) :: th
 integer :: i, s, j, numx
  ...
  if(i <= LATL-1 .or. LATL <= i) then !i <= LATL-1 .or. LATL <= i
    ...
    if(cvg(1, i) > cvg(3, i) .or. cvg(1, i) > cvg(2, i)) then                       ! 
      ...
      if(cvg(1, i) /= cvg(2, i) .and. cvg(1, i) /= cvg(3, i)) then
        CC = ((pep(2, i)-pep(1, i))/(cvg(2, i)-cvg(1, i))+(pep(1, i)-pep(3, i))/(cvg(1, i)-cvg(3, i)))/2.d0*cvg(1, i)*1.d1
       else if(cvg(1, i) == cvg(2, i) .and. cvg(1, i) /= cvg(3, i)) then
        CC = (pep(1, i)-pep(3, i))/(cvg(1, i)-cvg(3, i))*cvg(1, i)*1.d1
       else if(cvg(1, i) /= cvg(2, i) .and. cvg(1, i) == cvg(3, i)) then
        CC = (pep(1, i)-pep(2, i))/(cvg(1, i)-cvg(2, i))*cvg(1, i)*1.d1
      endif
      
!      if(cvg(1, i) > cvn*0.5) then !#
      
      if((abs(CC*DD*(cvg(1, i)/cvn)**2.d0) < 1.d-2 .or. pep(1, i) == 0.d0) .and. pep(1, i) < 0.d0) then
        npe(i) = npe(i) - CC*DD*(cvg(1, i)/cvn)**2.d0
       else if(abs(CC*DD*(cvg(1, i)/cvn)**2.d0) >= 1.d-2 .and. pep(1, i) < 0.d0) then
        npe(i) = npe(i) - CC*DD*(cvg(1, i)/cvn)**2.d0 / abs(CC*DD*(cvg(1, i)/cvn)**2.d0) * 1.d-2
       else if((abs(CC*DD*(cvg(1, i)/cvn)**2.d0) < 1.d1 .or. pep(1, i) == 0.d0) .and. &
       & pep(1, i) >= 0.d0 .and. pep(1, i) < 9.995d3) then
        npe(i) = npe(i) - CC*DD*cvg(1, i)/cvn
       else if(abs(CC*DD*(cvg(1, i)/cvn)**2.d0) >= 1.d1 .and. pep(1, i) >= 0.d0 .and. pep(1, i) < 9.995d3) then
        npe(i) = npe(i) - CC*DD*cvg(1, i)/cvn / abs(CC*DD*cvg(1, i)/cvn) * 1.d1
       else if((abs(CC*DD*(cvg(1, i)/cvn)**2.d0) < 1.d-2 .or. pep(1, i) == 0.d0) .and. pep(1, i) >= 9.995d3) then
        npe(i) = npe(i) - CC*DD*(cvg(1, i)/cvn)**2.d0
       else if(abs(CC*DD*(cvg(1, i)/cvn)**2.d0) >= 1.d-2 .and. pep(1, i) >= 9.995d3) then
        npe(i) = npe(i) - CC*DD*(cvg(1, i)/cvn)**2.d0 / abs(CC*DD*(cvg(1, i)/cvn)**2.d0) * 1.d-2
      endif
      
!      endif !#
      
      if(npe(i) < -1.d1 .or. npe(i) > 1.0002d4) then
        npe(i) = (pep(1, i+1)+pep(1, i-1))/2.d0
        if(npe(i) < -1.d1) then
          npe(i) = -1.d1
         else if(npe(i) > 1.0002d4) then
          npe(i) = 1.0002d4
        endif
      endif
 ...
 !MV~HATL-1で、常に静電ポテンシャルは電離圏に向かって増加すると仮定
 if(HATL-1 >= MV+1) then
   search = 1
   do while(search == 1)
    search = 0
 ...
 !HATL~LATL-1で、常に静電ポテンシャルは電離圏に向かって増加すると仮定
 if(LATL-1 >= HATL) then
   search = 1
   do while(search == 1)
    search = 0
 ...
 !LATL~N-1で、常に静電ポテンシャルは電離圏に向かって増加すると仮定
 if(N-1 >= LATL) then
   search = 1
   do while(search == 1)
    search = 0
...
! endif
 ...
 do i = 2, N-1
  write(70, 52) (pep(2, i)-pep(3, i)), (cvg(2, i)-cvg(3, i)), -((pep(2, i)-pep(1, i))/(cvg(2, i)-cvg(1, i))+&
 &(pep(1, i)-pep(3, i))/(cvg(1, i)-cvg(3, i)))/2.d0*cvg(1, i)*DD*(cvg(1, i)/cvn)**2.d0, &
