...
integer :: HATL = 110 !下段(not parameter)
integer :: LATL = 110 !上段(not parameter)、HATL <= LATL
integer, parameter :: kind = 6 !粒子種数
integer, parameter :: iodo = 2 !電離圏N側の数密度調整
integer, parameter :: mado = 5 !電離圏S側の数密度調整
...
!SVC_BCの抽出
open(20, file="SVC_BC_E_L=4_NS.csv", action="read", status="old")
read(20, *) !1行読み飛ばし
do i = 1, kind
...
open(33, file = "SVC_min_E_L=4_NS.csv")
read(33, *) DDD
!------------------------------
do i = 1, N
 read(33, *) pe0(i)
...
!つづきからはじめる▼
open(15, file="SVC_penum_E_L=4_NS.csv", action="read", status="old")
do i = 1, N
 read(15, *) CC
 pe0(i) = CC
enddo !i
...
!open(18, file="SVC_nsig_E_L=4_NS.csv", action="read", status="old")
!do s = 1, kind
! read(18, *) sigma(s)
!enddo !s
!close(18)
...
do !itn
 ...
 !ファイル化処理
 open(50, file = "SVC_penum_E_L=4_NS.csv")
 do i = 1, N
 ...
 open(60, file="SVC_pote_E_L=4_NS.csv")
 do i = 1, N
...
72 format(1PE25.15E3, 9(',', 1PE25.15E3)) !penum
...
subroutine EP(kind, N, ch, pe, GG, MassJ, mass, Req, LMD, omJ, dIo, MIo, UU, itn)
 ...
 86 format(1PE25.15E3, 5(',', 1PE25.15E3))
end subroutine EP
  ...
  do i = 1, N
   do s = 1, kind
    ...
    if(sigmab(s) == 1) then
      t = 1
      do k = 1, i
       if(UB(ud, k, s, j) > UB(ud, t, s, j)) t = k
      enddo !k
      a3(ud, i, s, j, 1) = sqrt(UB(ud, t, s, j) - UB(ud, sigmab(s), s, j))
    endif
     ...
     if(sigmab(s) == 1) then
       ...
       if(i /= N) then
         ...
         do k = i+1, N
          if(UB(ud, k, s, j) > UB(ud, t, s, j)) t = k
         enddo !k
         a3(ud, i, s, j, 3)  = UB(ud, t, s, j) - UB(ud, sigmab(s), s, j)
...
&((pep(2, i)-pep(1, i))/(cvg(2, i)-cvg(1, i))+(pep(3, i)-pep(1, i))/(cvg(3, i)-cvg(1, i)))/2.d0*cvg(1, i)/CC1*1.d0*DD
  if(i < HATL .and. CC1 > 1.d0 .and. sqrt(cvn) <= 1.d0) npe(i) = npe(i) - &
&((pep(2, i)-pep(1, i))/(cvg(2, i)-cvg(1, i))+(pep(3, i)-pep(1, i))/(cvg(3, i)-cvg(1, i)))/2.d0*cvg(1, i)/CC1*DD!*cvn**(-1./3.)
  if(i < HATL .and. CC1 <= 1.d0 .and. sqrt(cvn) > 1.d0) npe(i) = npe(i) - &
&((pep(2, i)-pep(1, i))/(cvg(2, i)-cvg(1, i))+(pep(3, i)-pep(1, i))/(cvg(3, i)-cvg(1, i)))/2.d0*cvg(1, i)*1.d0*DD
  if(i < HATL .and. CC1 <= 1.d0 .and. sqrt(cvn) <= 1.d0) npe(i) = npe(i) - &
&((pep(2, i)-pep(1, i))/(cvg(2, i)-cvg(1, i))+(pep(3, i)-pep(1, i))/(cvg(3, i)-cvg(1, i)))/2.d0*cvg(1, i)*DD!*cvn**(-1./3.)
...
&((pep(2, i)-pep(1, i))/(cvg(2, i)-cvg(1, i))+(pep(3, i)-pep(1, i))/(cvg(3, i)-cvg(1, i)))/2.d0*cvg(1, i)/CC3*1.d0*DD
  if(i >= LATL .and. CC3 > 1.d0 .and. sqrt(cvn) <= 1.d0) npe(i) = npe(i) - &
&((pep(2, i)-pep(1, i))/(cvg(2, i)-cvg(1, i))+(pep(3, i)-pep(1, i))/(cvg(3, i)-cvg(1, i)))/2.d0*cvg(1, i)/CC3*DD!*cvn**(-1./3.)
  if(i >= LATL .and. CC3 <= 1.d0 .and. sqrt(cvn) > 1.d0) npe(i) = npe(i) - &
&((pep(2, i)-pep(1, i))/(cvg(2, i)-cvg(1, i))+(pep(3, i)-pep(1, i))/(cvg(3, i)-cvg(1, i)))/2.d0*cvg(1, i)*1.d0*DD
  if(i >= LATL .and. CC3 <= 1.d0 .and. sqrt(cvn) <= 1.d0) npe(i) = npe(i) - &
&((pep(2, i)-pep(1, i))/(cvg(2, i)-cvg(1, i))+(pep(3, i)-pep(1, i))/(cvg(3, i)-cvg(1, i)))/2.d0*cvg(1, i)*DD!*cvn**(-1./3.)
...
&*sqrt(cvn)*DD
 ...
 open(44, file="SVC_nsig_E_L=4_NS.csv")
 do s = 1, kind
...
subroutine Alf(kind, N, mu0, BB, mass, ch, num, c)
 ...
 open(99, file="SVC_AW_E_L=4_NS.csv")
 do i = 1, N
