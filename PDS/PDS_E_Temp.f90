...
!mag_FAの抽出
open(50, file="mag_FA_E_Lhi.csv", action="read", status="old")
do i = 1, N
...
!SVC_parameterの抽出
open(51, file="SVC_parameter_E.csv", action="read", status="old")
read(51, *) dummy, ee
read(51, *) dummy, cc
read(51, *) 
read(51, *) dummy, mu0
...
!SVC_ICの抽出
open(52, file="PDS_IC_E_Lhi.csv", action="read", status="old")
read(52, *) !1行とばし
do i = 1, N
...
!SVC_BCの抽出
open(53, file="PDS_BC_E_Lhi_3.csv", action="read", status="old")
read(53, *) !1行とばし
do i = 1, kind
...
!SVC_minの抽出
open(54, file="PDS_E_Lhi_min_3.csv", action="read", status="old")
read(54, *) !1行とばし
do i = 1, N
 read(54, *) pe(i)
enddo !i
...
!ファイル化
open(60, file="PDS_E_Lhi_3_all_kai.csv")
do i = 1, N
...
enddo !i
close(60)
print *, "Finish PDS_E_Lhi_3_all_kai.csv"

open(61, file="PDS_E_Lhi_3_a3_all_kai.csv")
do s = 1, kind
 do j = 1, Z
  do i = 1, N
...
enddo !s
close(61)
print *, "Finish PDS_E_Lhi_3_a3_all_kai.csv"
...
70 format(1PE25.15E3, 101(',', 1PE25.15E3)) !10*kind+8*spece+18
...
subroutine disfun(N, kind, Z, sigmab, ee, BB, Tp, Ta, mass, sigma, mu, UU, a3)
 ...
 do s = 1, kind
  do i = 1, N
   ...
   write(filename1, *) s
   write(filename2, *) i
   filename = 'PDS_E_Lhi_3_disfun_s='//trim(adjustl(filename1))//'_i='//trim(adjustl(filename2))//'.csv'
   print *, filename
...
subroutine disfun2(N, kind, Z, sigmab, ee, BB, Tp, Ta, mass, sigma, mu, UU, a3)
 ...
 do s = 1, kind
  do j = 1, Z
   ...
   filename = 'PDS_E_Lhi_3_disfun_s='//trim(adjustl(filename1))//'_mu='//trim(adjustl(filename2))//'.csv'
   print *, filename
