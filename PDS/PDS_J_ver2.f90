...
integer :: MV = 31 !極小値(not parameter)
integer :: HATL = 68 !下段(not parameter)
integer :: LATL = 68 !上段(not parameter)、HATL <= LATL
...
!SVC_BCの抽出
open(20, file="SVC_BC_ehot.csv", action="read", status="old")
read(20, *) !1行読み飛ばし
do i = 1, kind
...
open(33, file = "PDS_J_30kV_min_kai_ehot.csv")
read(33, *) DDD
!------------------------------
do i = 1, N
 read(33, *) pe0(i)
...
!      if(cvg(1, i) > cvn*0.5) then !#
      ...
      if((abs(CC*DD*(cvg(1, i)/cvn)**2.d0) < 1.d-1 .or. pep(1, i) == 0.d0) .and. pep(1, i) < 0.d0) then
        npe(i) = npe(i) - CC*DD*(cvg(1, i)/cvn)**2.d0
       else if(abs(CC*DD*(cvg(1, i)/cvn)**2.d0) >= 1.d-1 .and. pep(1, i) < 0.d0) then
        npe(i) = npe(i) - CC*DD*(cvg(1, i)/cvn)**2.d0 / abs(CC*DD*(cvg(1, i)/cvn)**2.d0) * 1.d-1
       else if((abs(CC*DD*(cvg(1, i)/cvn)**2.d0) < 1.d0 .or. pep(1, i) == 0.d0) .and. &
       & pep(1, i) >= 0.d0 .and. pep(1, i) < 2.999d4) then
        npe(i) = npe(i) - CC*DD*cvg(1, i)/cvn
       else if(abs(CC*DD*(cvg(1, i)/cvn)**2.d0) >= 1.d1 .and. pep(1, i) >= 0.d0 .and. pep(1, i) < 2.999d4) then
        npe(i) = npe(i) - CC*DD*cvg(1, i)/cvn / abs(CC*DD*cvg(1, i)/cvn) * 1.d1
       else if((abs(CC*DD*(cvg(1, i)/cvn)**2.d0) < 1.d-2 .or. pep(1, i) == 0.d0) .and. pep(1, i) >= 2.999d4) then
        npe(i) = npe(i) - CC*DD*(cvg(1, i)/cvn)**2.d0
       else if(abs(CC*DD*(cvg(1, i)/cvn)**2.d0) >= 1.d-2 .and. pep(1, i) >= 2.999d4) then
        npe(i) = npe(i) - CC*DD*(cvg(1, i)/cvn)**2.d0 / abs(CC*DD*(cvg(1, i)/cvn)**2.d0) * 1.d-2
      endif
      
!      endif !#
      
      if(npe(i) < -5.d1 .or. npe(i) > 3.0002d4) then
        npe(i) = (pep(1, i+1)+pep(1, i-1))/2.d0
        if(npe(i) < -5.d1) then
          npe(i) = -5.d1
         else if(npe(i) > 3.0002d4) then
          npe(i) = 3.0002d4
        endif
      endif
