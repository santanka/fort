...
subroutine Newtonphi(N, pep, cvg, npe, c, m, e, cvn, itn)
 ...
 !if(CC >= 1.d0) then
   do i = 2, N-1
    if(i <= 55) npe(i) = npe(i) - &
&((pep(2, i)-pep(1, i))/(cvg(2, i)-cvg(1, i))+(pep(3, i)-pep(1, i))/(cvg(3, i)-cvg(1, i)))/2.d0*cvg(1, i)/CC1/1.d2*cvn
    if(i > 55) npe(i) = npe(i) - &
&((pep(2, i)-pep(1, i))/(cvg(2, i)-cvg(1, i))+(pep(3, i)-pep(1, i))/(cvg(3, i)-cvg(1, i)))/2.d0*cvg(1, i)/CC2/1.d2*cvn
   enddo !i
 !endif
! if(CC <= 1.d0) then
!   do i = 2, N-1
!    !if(i <= 55 .and. i /= mxn) npe(i) = npe(i) - &
!&((pep(2, i)-pep(1, i))/(cvg(2, i)-cvg(1, i))+(pep(3, i)-pep(1, i))/(cvg(3, i)-cvg(1, i)))/2.d0*cvg(1, i)*cvn*1.d1
!    if(i > 55 .and. i /= mxn) npe(i) = npe(i) - &
!&((pep(2, i)-pep(1, i))/(cvg(2, i)-cvg(1, i))+(pep(3, i)-pep(1, i))/(cvg(3, i)-cvg(1, i)))/2.d0*cvg(1, i)*cvn/1.d1
!    if(i == mxn) npe(i) = npe(i) - &
!&((pep(2, i)-pep(1, i))/(cvg(2, i)-cvg(1, i))+(pep(3, i)-pep(1, i))/(cvg(3, i)-cvg(1, i)))/2.d0*cvg(1, i)*cvn/1.d1
!  enddo !i
! endif
...
&npe(i)-pep(1, i), npe(i)
 ...
 print *, mod(itn, 77)+1, npe(55)-pep(1, 55)!, mxn, npe(mxn)-pep(1, mxn)
 print *, "  "
 return
