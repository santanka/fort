...
!mag_FAの抽出
open(40, file="mag_FA_E.csv", action="read", status="old")
do i = 1, N
...
!SVC_ICの抽出
open(10, file="SVC_IC_E.csv", action="read", status="old")
read(10, *) !1行読み飛ばし
do i = 1, N
...
!SVC_BCの抽出
open(20, file="SVC_BC_E.csv", action="read", status="old")
read(20, *) !1行読み飛ばし
do i = 1, 8
...
do !itn
 ...
 if(DDD > cvn) then
   DDD = cvn
   open(33, file = "SVC_min.csv")
   write(33, '(E25.15E3)') cvn
   do i = 1, N
    write(33, '(E25.15E3)') pep(1, i)
   enddo !i
   do s = 1, 8
    write(33, '(E25.15E3)') sigma(s)
   enddo !s
   close(33)
 endif
...
subroutine pepm(pep, e, m, c, N, UU, ch)
 ...
 do i = 1, N
  if(i <= 71) dp(i) = pep(1, i)*1.d-2
  !if(i >= 61 .and. i <= 71) dp(i) = pep(1, i)*1.d-4
  if(i >= 72) dp(i) = pep(1, i)*1.d-7
 enddo !i
  ...
  do i = 1, N
   do s = 1, 8
    do j = 1, Z       
     ...
     if((i == N .and. s == 3) .or. (i == 1 .and. s == 6)) then
       if(s == 3) numx = 1
       if(s == 6) numx = 2
...
subroutine CV(N, ch, rhov, num, cvg, cvn)
 ...
 do ud = 1, 3
  do i = 1, N
   nume(ud, i) = abs(ch(3)*num(ud, i, 3) + ch(4)*num(ud, i, 4) + ch(5)*num(ud, i, 5) &
    & + ch(8)*num(ud, i, 8))
   numi(ud, i) = ch(1)*num(ud, i, 1) + ch(2)*num(ud, i, 2) + ch(6)*num(ud, i, 6) &
    & + ch(7)*num(ud, i, 7)
  enddo !i
 enddo !ud
...
!Newton法(npe)
subroutine Newtonphi(N, pep, cvg, npe, c, m, e, cvn, itn)
 implicit none
 integer, intent(in) :: N, itn
 ...
 enddo
   ...
   do i = 2, N-1
    if(i <= 71) npe(i) = npe(i) - &
&((pep(2, i)-pep(1, i))/(cvg(2, i)-cvg(1, i))+(pep(3, i)-pep(1, i))/(cvg(3, i)-cvg(1, i)))/2.d0*cvg(1, i)/CC1*cvn*1.d2
!    if(i > 60 .and. i <= 71) npe(i) = npe(i) - &
!&((pep(2, i)-pep(1, i))/(cvg(2, i)-cvg(1, i))+(pep(3, i)-pep(1, i))/(cvg(3, i)-cvg(1, i)))/2.d0*cvg(1, i)/CC2*cvn
    if(i > 71) npe(i) = npe(i) - &
&((pep(2, i)-pep(1, i))/(cvg(2, i)-cvg(1, i))+(pep(3, i)-pep(1, i))/(cvg(3, i)-cvg(1, i)))/2.d0*cvg(1, i)/CC3*cvn/1.d2
   enddo !i
...
&npe(i)-pep(1, i), npe(i)
 ...
 print *, mod(itn, 77)+1, npe(49)-pep(1, 49)!, mxn, npe(mxn)-pep(1, mxn)
 print *, "  "
 return
 
 52 format(E25.15E3, 7(',', E25.15E3))
