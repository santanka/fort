...
integer :: HATL = 204 !下段(not parameter)
integer :: LATL = 211 !上段(not parameter)、HATL <= LATL
...
!mag_FAの抽出
open(40, file="mag_FA_E_Lhi.csv", action="read", status="old")
do i = 1, N
...
!SVC1_parameterの抽出
open(30, file="SVC_parameter_E.csv", action="read", status="old")
do i = 1, 12
...
!SVC_ICの抽出
open(10, file="SVC_IC_E_Lhi_211_204.csv", action="read", status="old")
read(10, *) !1行読み飛ばし
do i = 1, N
...
!SVC_BCの抽出
open(20, file="SVC_BC_E_Lhi_01eV_211_204_05.csv", action="read", status="old")
read(20, *) !1行読み飛ばし
do i = 1, kind
...
open(33, file = "SVC_min_E_Lhi_01eV_211_204_05.csv")
read(33, *) DDD
!------------------------------
do i = 1, N
 read(33, *) pe0(i)
...
!つづきからはじめる▼
!open(15, file="SVC_penum_E_Lhi_01eV_211_204_05.csv", action="read", status="old")
!do i = 1, N
! read(15, *) CC
! pe0(i) = CC
!enddo !i
...
!open(18, file="SVC_nsig_E_Lhi_01eV_211_204_05.csv", action="read", status="old")
!do s = 1, kind
! read(18, *) sigma(s)
!enddo !s
!close(18)
...
do !itn
 ...
 !ファイル化処理
 open(50, file = "SVC_penum_E_Lhi_01eV_211_204_05.csv")
 do i = 1, N
 ...
 open(60, file="SVC_pote_E_Lhi_01eV_211_204_05.csv")
 do i = 1, N
...
subroutine pepm(kind, LATL, HATL, pep, e, m, c, N, UU, ch, DD)
 ...
 do i = 1, N
  if(abs(pep(1, i)) >= 1.d1 .and. i < LATL) dp(i) = 1.d-3*DD
  if(abs(pep(1, i)) < 1.d1 .and. i < LATL) dp(i) = 1.d-4*pep(1, i)*DD
  if(i >= LATL) dp(i) = 1.d-7*DD
!  if(i == HATL) dp(i) = 5.d1
 enddo !i
...
subroutine Newtonphi(LATL, HATL, N, pep, cvg, npe, c, m, e, cvn, itn, check, DD, XX)
 ...
 integer :: i, mxn, kyokuchi, serch, che
...
!CC3の調整
  if(i >= LATL .and. CC3 > 1.d0 .and. sqrt(cvn) > 1.d-2) npe(i) = npe(i) - &
&((pep(2, i)-pep(1, i))/(cvg(2, i)-cvg(1, i))+(pep(3, i)-pep(1, i))/(cvg(3, i)-cvg(1, i)))/2.d0*cvg(1, i)/CC3*1.d-2*DD
  if(i >= LATL .and. CC3 > 1.d0 .and. sqrt(cvn) <= 1.d-2) npe(i) = npe(i) - &
&((pep(2, i)-pep(1, i))/(cvg(2, i)-cvg(1, i))+(pep(3, i)-pep(1, i))/(cvg(3, i)-cvg(1, i)))/2.d0*cvg(1, i)/CC3*sqrt(cvn)*DD
  if(i >= LATL .and. CC3 <= 1.d0 .and. sqrt(cvn) > 1.d-2) npe(i) = npe(i) - &
&((pep(2, i)-pep(1, i))/(cvg(2, i)-cvg(1, i))+(pep(3, i)-pep(1, i))/(cvg(3, i)-cvg(1, i)))/2.d0*cvg(1, i)*1.d-2*DD
  if(i >= LATL .and. CC3 <= 1.d0 .and. sqrt(cvn) <= 1.d-2) npe(i) = npe(i) - &
&((pep(2, i)-pep(1, i))/(cvg(2, i)-cvg(1, i))+(pep(3, i)-pep(1, i))/(cvg(3, i)-cvg(1, i)))/2.d0*cvg(1, i)*sqrt(cvn)*DD
...
!  if(i == HATL-1 .and. kyoku(i) == 1 .and. i /= kyokuchi) then
 ...
 if(HATL < LATL) then
  ...
  if(npe(LATL) < ((npe(LATL+1)+npe(N))/2.d0+npe(LATL-1))/2.d0 .and. npe(LATL) < 3.d0*npe(LATL+1)-2.d0*npe(LATL+2)) then
    LATL = LATL + 1
    print *, "LATL is changed to ", LATL
  endif
  if(npe(LATL-1) > ((npe(LATL)+npe(N))/2.d0+npe(LATL-2))/2.d0 .and. npe(LATL-1) > 3.d0*npe(LATL)-2.d0*npe(LATL+1)) then
    LATL = LATL - 1
    print *, "LATL is changed to ", LATL
  endif
 endif
...
&*sqrt(cvn)*DD
 ...
 open(44, file="SVC_nsig_E_Lhi_01eV_211_204_05.csv")
 do s = 1, kind
...
subroutine Alf(kind, N, mu0, BB, mass, ch, num, c)
 ...
 open(99, file="SVC_AW_E_Lhi_01eV_211_204_05.csv")
 do i = 1, N
