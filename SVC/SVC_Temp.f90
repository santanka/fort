...
!muについて積分(数密度)
call NM1(N, kind, Z, BB, mass, mu, theta1, sigmab, num)
...
!muについて積分(温度)
call NM3(N, kind, Z, ee, BB, mass, mu, theta3, num, sigmab, Tperp, Tpara)
...
!ファイル化
open(60, file="SVC_E_L=4_NS_all_kai.csv")
do i = 1, N
...
open(61, file="SVC_E_L=4_NS_a3_all_kai.csv")
do s = 1, kind
 do j = 1, Z
  write(61, 71) "s = ", s, "j = ", j
  do i = 1, N
...
!muについて積分(数密度)
subroutine NM1(N, kind, Z, BB, mass, mu, theta1, sigmab, num)
 implicit none
 integer, intent(in) :: N, kind, Z
 real*8, dimension(N), intent(in) :: BB
 ...
 do i = 1, N
  do s = 1, kind
   ...
   num(i, s) = sqrt((2.d0/mass(s))**3.d0)*BB(sigmab(s))*pi*nnn
...
subroutine NM2(N, kind, Z, BB, mass, mu, theta2, num, sigmab, aave)
 ...
 do i = 1, N
  do s = 1, kind
   ...
   aave(i, s) = (2.d0/mass(s))**(3.d0/2.d0)*pi*BB(sigmab(s))/num(i, s)*nnn
...
!muについて積分(温度)
subroutine NM3(N, kind, Z, ee, BB, mass, mu, theta3, num, sigmab, Tperp, Tpara)
 implicit none
 integer, intent(in) :: N, kind, Z
 real*8, intent(in) :: ee
 real*8, dimension(N), intent(in) :: BB
 ...
 do i = 1, N
  do s = 1, kind
   ...
   Tperp(i, s) = 2.d0*sqrt(2.d0/mass(s)**3.d0)*(BB(sigmab(s))**2.d0)*pi/num(i, s)*nnn /ee
   ...
   Tpara(i, s) = 4.d0*sqrt(2.d0/mass(s)**3.d0)*BB(sigmab(s))*pi/num(i, s)*nnn /ee
