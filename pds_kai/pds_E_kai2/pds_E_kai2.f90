...
integer, parameter :: CP = 10000 !boundary以外の静電ポテンシャル無変化点
...
double precision, parameter :: Lp = 4.d0 !対象の磁力線のL値
...
character(len=128) :: fileBC = 'pds_BC_E_kai2_1.csv'
...
character(len=128) :: fileIC = 'pds_IC_E_kai2_1.csv'
...
character(len=128) :: fileresult = 'pds_E_kai2_1_result.csv'
72 format(1PE25.15E3, 18(',', 1PE25.15E3)) !kind+7
character(len=128) :: filepote = 'pds_E_kai2_1_potential.csv'
82 format(1PE25.15E3, 15(',', 1PE25.15E3)) !kind+4
character(len=128) :: filemin = 'pds_E_kai2_1_min.csv'
92 format(1PE25.15E3) !1
character(len=128) :: filecheck = 'pds_E_kai2_1_check.csv'
 ...
 do h = 1, 3
  do i = 1, N
   if(i /= 1 .and. i /= N) then
     ...
     enddo !j
     nd(h, i, :) = sigg(1, :)*BB(i)/2.d0/Tperp*nd(h, i, :) !BB(ijn) -> BB(i)
    else if(i == 1 .or. i == N) then
     do j = 1, Z-1
      nd(h, i, :) = nd(h, i, :) + (TT(1, i, :, j)+TT(1, i, :, j+1))/2.d0*(mu(:, j+1)-mu(:, j))
     enddo !j
     nd(h, i, :) = sigg(h, :)*BB(i)/2.d0/Tperp*nd(h, i, :) !BB(ijn) -> BB(i)
   endif
  enddo !i
 enddo !h
...
subroutine NewtonPhi(N, MV, CP, Phih, cvg, nPhi)
 ...
 do i = 2, N-1
  ...
  if((abs(CC) < cvg(1, i) .and. cvg(1, i) <= 1.d0) .and. i /= CP) then
    nPhi(i) = Phih(1, i) - CC
