...
!保存ファイル
character(len=128) :: fileall = 'pds_E_kai2_1_all.csv'
...
character(len=128) :: fileoption = 'pds_E_kai2_1_disfun_'
 ...
 do i = 1, N
  do s = 1, kind
   if(nd(i, s) == 0.d0) then
     ...
     Vpara(i, s) = sqrt(2.d0/pi/mass(s)/Tpara(s))*sig(s)*BB(i)/Tperp(s)/nd(i, s) * nnn !BB(ijn(s)) -> BB(i)
   endif
  enddo !s
 enddo !i
...
subroutine pressureperp(N, Z, kind, mu, BB, Tperp, Tpara, sig, amin, amax, ijn, Pperp)
 ...
 do s = 1, kind
  ...
  enddo !j
  Pperp(:, s) = sig(s)*BB(:)*BB(:)/Tperp(s)*Pperp(:, s) !BB(ijn(s)) -> BB(:)
 enddo !s
...
subroutine pressurepara(N, Z, kind, UU, Vpara, mu, BB, mass, Tpara, Tperp, sig, amin, alim, amax, ijn, Ppara)
 ...
 do i = 1, N
  do s = 1, kind
   ...
   Ppara(i, s) = sig(s)*BB(i)*mass(s)/Tperp(s)/sqrt(pi*Tpara(s)) * nnn !BB(ijn(s)) -> BB(i)
  enddo !s
 enddo !i
