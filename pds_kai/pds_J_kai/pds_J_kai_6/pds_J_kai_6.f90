...
character(len=128) :: fileBC = 'pds_BC_J_kai_6.csv'
...
character(len=128) :: fileIC = 'pds_IC_J_kai_6.csv'
...
double precision, dimension(91) :: cvn_min !収束値データ(27~(N+1)/2-1)



!保存ファイル名
character(len=128) :: filetop = 'pds_J_kai_6_'
...
character(len=128) :: file_cvn = 'pds_J_kai_6_cvn.csv'
...
do MV_itn = 27, (N+1)/2-1
 ...
 cvn_min(MV_itn-27+1) = DDD
 
 !収束値データ化
 if(MV_itn == (N+1)/2-1) then
   open(80, file = file_cvn)
   do i = 31, (N+1)/2-1 !MV_itnの範囲
    write(80, 62) lam((N+1)/2+i-30), ss((N+1)/2+i-30), BB((N+1)/2+i-30), cvn_min((N+1)/2-i+1)
   enddo !i
   close(80)
  endif
