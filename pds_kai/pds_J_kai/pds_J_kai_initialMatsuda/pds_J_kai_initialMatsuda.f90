...
integer, parameter :: MV_fix = 117 !極大値となる座標
integer, parameter :: CP = 117 !boundary以外の静電ポテンシャル無変化点
integer, parameter :: roler = 50 !ローラー作戦スタート地点
...
double precision, parameter :: lamN = acos(sqrt((1.d0+25.d5/Rp)/Lp)) !boundary:NのMLT
double precision, parameter :: lamS = -acos(sqrt((1.d0+25.d5/Rp)/Lp)) !boundary:SのMLT
...
character(len=128) :: fileBC = 'pds_BC_J_kai_initialMatsuda.csv'
...
double precision, dimension(kind) :: sig, sigfix !数密度
...
character(len=128) :: fileIC = 'pds_IC_J_kai_initialMatsuda.csv'
...
double precision, dimension(N) :: cvn_min !収束値データ



!保存ファイル名
character(len=128) :: filetop = 'pds_J_kai_initialMatsuda_'
...
character(len=128) :: file_cvn = 'pds_J_kai_initialMatsuda_cvn.csv'
...
do MV_itn = roler, (N+1)/2-1
 ...
 if(MV_itn == (N+1)/2-1) then
   ...
   do i = roler, (N+1)/2-1 !MV_itnの範囲
    write(80, 62) lam(N-i), ss(N-i), BB(N-i), cvn_min(i)
   enddo !i
   close(80)
  endif
 ...
 do i = 2, N-1
  ...
  if(i /= CP) then
    ...
    if(abs(CC) < cvg(1, i) .and. cvg(1, i) <= 1.d0) then
     ...
     else if(abs(CC) < 5.d0 .and. cvg(1, i) > 1.d0) then
      nPhi(i) = Phih(1, i) - CC
     else if(abs(CC) >= 5.d0 .and. cvg(1, i) > 1.d0) then
...
subroutine Newtonsig(N, kind, ijn, sigg, cvg, nsig, sigfix)
 ...
 do s = 1, kind
  ...
  if(nsig(s) < sigfix(s)/5d0 .or. nsig(s) > sigfix(s)*5d0) then
    nsig(s) = sigfix(s)
  endif
