...
character(len=128) :: filemin = 'pds_J_kai_initialMatsuda_2_cvnminpoint_75_min.csv'
...
!保存ファイル
character(len=128) :: fileall = 'pds_J_kai_initialMatsuda_2_cvnminpoint_75_all.csv'
...
character(len=128) :: fileoption = 'pds_J_kai_initialMatsuda_2_cvnminpoint_75_disfun_'
   ...
   if(mod(j, 3) == 1) then
     ...
     do p = 1, Z/2
      if(mod(p, 3) == 2 .and. nd(N1, s) /= 0.d0) then
        ff = sig(s)/nd(N1, s)*(mass(s)/2.d0/pi)**(3.d0/2.d0)/Tperp(s)/sqrt(Tpara(s)) &
  ...
  do i = 1, N
   ...
   do p = 1, Z/2
    if(mod(p, 3) == 2 .and. nd(i, s) /= 0.d0) then
      ff = sig(s)/nd(i, s)*(mass(s)/2.d0/pi)**(3.d0/2.d0)/Tperp(s)/sqrt(Tpara(s)) &
