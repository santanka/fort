...
integer, parameter :: MV_itn = 55 !極小値
integer :: trigger !iteration終了
...
character(len=128) :: fileBC = 'pds_BC_J_kai_initialMatsuda_2.csv'
...
character(len=128) :: fileIC = 'pds_IC_J_kai_initialMatsuda_2.csv'
...
!保存ファイル名
character(len=128) :: filetop = 'pds_J_kai_initialMatsuda_2_cvnminpoint'
  ...
  if(i /= CP) then
    ...
    if(abs(CC) < cvg(1, i) .and. cvg(1, i) <= 1.d0) then
      ...
      if(Phih(1, i) > 0.d0 .and. Phih(1, i) < 2.9999d4) then
        nPhi(i) = Phih(1, i) - CC/abs(CC)*1.d2*sqrt(cvg(1, i))
       else
        if((abs(CC) < cvg(1, i) .and. cvg(1, i) <= 1.d-4) .and. i /= CP) then
          nPhi(i) = Phih(1, i) - CC
         else if(abs(CC) >= cvg(1, i) .and. cvg(1, i) <= 1.d-4 .and. i /= CP) then
     ...
     else if(abs(CC) < 1.d2 .and. cvg(1, i) > 1.d0) then
      nPhi(i) = Phih(1, i) - CC
     else if(abs(CC) >= 1.d2 .and. cvg(1, i) > 1.d0) then
      nPhi(i) = Phih(1, i) - CC/abs(CC)*1.d2
    endif
  endif
 enddo !i
